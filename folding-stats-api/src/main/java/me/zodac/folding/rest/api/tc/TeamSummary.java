...
public class TeamSummary {
     ...
     */
    public static TeamSummary create(final String teamName, final String teamDescription, final String forumLink, final String captainName, final Collection<UserSummary> activeUsers, final Collection<RetiredUserSummary> retiredUsers) {
        int teamUnits = 0;
        long teamPoints = 0L;
        ...
        final List<RetiredUserSummary> rankedRetiredUsers = retiredUsers
                ...
                .stream()
                .map(rankedRetiredUser -> RetiredUserSummary.updateWithRankInTeam(rankedRetiredUser, rankedRetiredUser.getRankInTeam() + activeUsers.size()))
                .collect(toList());

        // Not ranked to begin with, will be updated by the calling class
        return new TeamSummary(teamName, teamDescription, forumLink, captainName, teamPoints, teamMultipliedPoints, teamUnits, DEFAULT_TEAM_RANK, rankedActiveUsers, rankedRetiredUsers);
    }
    ...
    public static TeamSummary updateWithRank(final TeamSummary teamSummary, final int rank) {
        return new TeamSummary(teamSummary.teamName, teamSummary.teamDescription, teamSummary.forumLink, teamSummary.captainName, teamSummary.teamPoints, teamSummary.teamMultipliedPoints, teamSummary.teamUnits, rank, teamSummary.activeUsers, teamSummary.retiredUsers);
    }
}