...
function getUserHistoricStats(a,c,b,e,d,f){0!=a&&(selectedUserId=a);null!=c&&(selectedUser=c,userDropdownTitle=document.getElementById("user_dropdown_root"),userDropdownTitle.innerHTML=selectedUser);null!=b&&(selectedDay=b,dayDropdownTitle=document.getElementById("day_dropdown_root"),dayDropdownTitle.innerHTML=ordinalSuffixOf(selectedDay));null!=e&&(selectedMonth=e);null!=d&&(selectedMonthName=d,monthDropdownTitle=document.getElementById("month_dropdown_root"),monthDropdownTitle.innerHTML=selectedMonthName);
null!=f&&(selectedYear=f,yearDropdownTitle=document.getElementById("year_dropdown_root"),yearDropdownTitle.innerHTML=selectedYear);""!=selectedDay&&null!=selectedDay&&populateDayDropdown(selectedMonth,selectedYear,"day_dropdown","getUserHistoricStats");""!==selectedUser&&0!==selectedUserId&&(show("loader"),hide("historic_stats"),fetch(ROOT_URL+"/historic/users/"+selectedUserId+"/"+selectedYear+"/"+selectedMonth+"/"+selectedDay).then(function(a){return a.json()}).then(function(a){for(historicDiv=document.getElementById("historic_stats");historicDiv.firstChild;)historicDiv.removeChild(historicDiv.lastChild);
...
document.addEventListener("DOMContentLoaded",function(a){populateUserDropdown("user_dropdown");populateDayDropdown(selectedMonth,selectedYear,"day_dropdown","getUserHistoricStats");populateMonthDropdown("month_dropdown","getUserHistoricStats");populateYearDropdown("year_dropdown","getUserHistoricStats");updateTimer()});function show(a){document.getElementById(a).style.display="block"}function showElement(a){a.style.display="block"}function hide(a){document.getElementById(a).style.display="none"}
