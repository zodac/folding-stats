...
function populateDropdown(){console.log("Starting");$("#user_dropdown").empty();fetch(ROOT_URL+"/users").then(function(a){return a.json()}).then(function(a){dropdown_div=document.getElementById("user_dropdown");$.each(a,function(a,b){userButton=document.createElement("button");userButton.setAttribute("class","dropdown-item");userButton.setAttribute("type","button");userButton.setAttribute("onclick","getUserHistoricStats("+b.id+",'"+b.displayName+"')");userButton.innerHTML=b.displayName;dropdown_div.append(userButton)})})}
function getUserHistoricStats(a,c){$("#loader").show();$("#historic_stats").hide();var b=new Date,f=b.getFullYear(),d=b.getMonth()+1,e=b.toLocaleString("default",{month:"long"});fetch(ROOT_URL+"/historic/users/"+a+"/"+f+"/"+d).then(function(a){return a.json()}).then(function(a){dropDownTitle=document.getElementById("user_dropdown_root");dropDownTitle.innerHTML=c;for(historicDiv=document.getElementById("historic_stats");historicDiv.firstChild;)historicDiv.removeChild(historicDiv.lastChild);userTitle=
...
"sortTable("+b+", 'historic_table')");tableHeader.setAttribute("scope","col");tableHeader.innerHTML=a;tableHeaderRow.append(tableHeader)});tableHead.append(tableHeaderRow);historicTable.append(tableHead);tableBody=document.createElement("tbody");a.forEach(function(a,b){tableRow=document.createElement("tr");dateCell=document.createElement("td");dateCell.innerHTML=ordinalSuffixOf(a.date.day);tableRow.append(dateCell);pointsCell=document.createElement("td");pointsCell.setAttribute("data-toggle","tooltip");
