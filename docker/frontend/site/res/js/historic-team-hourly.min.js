var ROOT_URL="http://internal.axihub.ca/folding",currentDate=new Date,currentUtcDate=new Date(currentDate.getUTCFullYear(),currentDate.getUTCMonth(),currentDate.getUTCDate(),currentDate.getUTCHours(),currentDate.getUTCMinutes(),currentDate.getUTCSeconds()),selectedTeamId=0,selectedTeam="",selectedMonth=currentUtcDate.getMonth()+1,selectedYear=currentUtcDate.getFullYear(),selectedMonthName=(new Date(selectedYear,selectedMonth-1,1)).toLocaleString("default",{month:"long"}),selectedDay=currentDate.getUTCDate();
function getTeamHistoricStats(a,c,b,e,d,f){0!=a&&(selectedTeamId=a);null!=c&&(selectedTeam=c,teamDropdownTitle=document.getElementById("team_dropdown_root"),teamDropdownTitle.innerHTML=selectedTeam);null!=b&&(selectedDay=b,dayDropdownTitle=document.getElementById("day_dropdown_root"),dayDropdownTitle.innerHTML=ordinalSuffixOf(selectedDay));null!=e&&(selectedMonth=e);null!=d&&(selectedMonthName=d,monthDropdownTitle=document.getElementById("month_dropdown_root"),monthDropdownTitle.innerHTML=selectedMonthName);
null!=f&&(selectedYear=f,yearDropdownTitle=document.getElementById("year_dropdown_root"),yearDropdownTitle.innerHTML=selectedYear);""!==selectedTeam&&0!==selectedTeamId&&(show("loader"),hide("historic_stats"),fetch(ROOT_URL+"/historic/teams/"+selectedTeamId+"/"+selectedYear+"/"+selectedMonth+"/"+selectedDay).then(function(a){return a.json()}).then(function(a){for(historicDiv=document.getElementById("historic_stats");historicDiv.firstChild;)historicDiv.removeChild(historicDiv.lastChild);historicTable=
document.createElement("table");historicTable.setAttribute("id","historic_table");historicTable.setAttribute("class","table table-dark table-striped table-hover");tableHead=document.createElement("thead");tableHeaderRow=document.createElement("tr");["Hour","Points","Units"].forEach(function(a,b){tableHeader=document.createElement("th");tableHeader.setAttribute("onclick","sortTable("+b+", 'historic_table')");tableHeader.setAttribute("scope","col");tableHeader.innerHTML=a;tableHeaderRow.append(tableHeader)});
tableHead.append(tableHeaderRow);historicTable.append(tableHead);tableBody=document.createElement("tbody");a.forEach(function(a,b){tableRow=document.createElement("tr");dateCell=document.createElement("td");dateCell.innerHTML=leftPad(a.dateTime.time.hour,2,"0")+":00";tableRow.append(dateCell);pointsCell=document.createElement("td");pointsCell.setAttribute("data-bs-toggle","tooltip");pointsCell.setAttribute("data-placement","top");pointsCell.setAttribute("title","Unmultiplied: "+a.points.toLocaleString());
...
document.addEventListener("DOMContentLoaded",function(a){populateTeamDropdown("team_dropdown");populateDayDropdown(selectedMonth,selectedYear,"day_dropdown","getTeamHistoricStats");populateMonthDropdown("month_dropdown","getTeamHistoricStats");populateYearDropdown("year_dropdown","getTeamHistoricStats");updateTimer()});function show(a){document.getElementById(a).style.display="block"}function showElement(a){a.style.display="block"}function hide(a){document.getElementById(a).style.display="none"}
function hideElement(a){a.style.display="none"}var NUMBER_OF_MONTHS=12,NUMBER_OF_YEARS_TO_SHOW=2,YEAR_START=(new Date).getUTCFullYear();
function populateUserDropdown(a){for(var c=document.getElementById(a);c.firstChild;)c.removeChild(c.lastChild);fetch(ROOT_URL+"/users").then(function(a){return a.json()}).then(function(b){userDropdownDiv=document.getElementById(a);b.forEach(function(a,b){userButton=document.createElement("button");userButton.setAttribute("class","dropdown-item");userButton.setAttribute("type","button");userButton.setAttribute("onclick","getUserHistoricStats("+a.id+",'"+JSON.stringify(a.displayName)+"',null,null,null,null)");
userButton.innerHTML=a.displayName;userDropdownDiv.append(userButton)})})}
function populateTeamDropdown(a){for(var c=document.getElementById(a);c.firstChild;)c.removeChild(c.lastChild);fetch(ROOT_URL+"/teams").then(function(a){return a.json()}).then(function(b){teamDropDownDiv=document.getElementById(a);b.forEach(function(a,b){teamButton=document.createElement("button");teamButton.setAttribute("class","dropdown-item");teamButton.setAttribute("type","button");teamButton.setAttribute("onclick","getTeamHistoricStats("+a.id+",'"+JSON.stringify(a.teamName)+"',null,null,null,null)");
...
"_root");yearDropdownTitle.innerHTML=selectedYear}function ordinalSuffixOf(a){var c=a%10,b=a%100;return 1==c&&11!=b?a+"st":2==c&&12!=b?a+"nd":3==c&&13!=b?a+"rd":a+"th"}function leftPad(a,c,b){return(String(b).repeat(c)+String(a)).slice(String(a).length)}
