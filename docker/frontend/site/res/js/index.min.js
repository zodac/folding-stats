var ROOT_URL="http://127.0.0.1:8080/folding";function toggleMainButtonStyle(a,b){a=document.getElementById(a);b.contains("collapsed")?(a.classList.add("btn-primary"),a.classList.remove("btn-success")):(a.classList.add("btn-success"),a.classList.remove("btn-primary"))}function toggleTeam(a,b){a=document.getElementById("team_"+a+"_button");b.contains("collapsed")?(a.classList.add("btn-primary"),a.classList.remove("btn-success")):(a.classList.add("btn-success"),a.classList.remove("btn-primary"))}
function loadOverallStats(){fetch(ROOT_URL+"/stats").then(function(a){return a.json()}).then(function(a){overallDiv=document.getElementById("overall_div");overallTitle=document.createElement("h2");overallTitle.setAttribute("class","navbar-brand");overallTitle.innerHTML="Overall Stats";overallDiv.append(overallTitle);overallTable=document.createElement("table");overallTable.setAttribute("id","overall");overallTable.setAttribute("class","table table-dark table-striped table-hover");overallTableHead=
...
function loadCategoryLeaderboard(){fetch(ROOT_URL+"/stats/category").then(function(a){return a.json()}).then(function(a){categoryDiv=document.getElementById("category_div");var b="Rank;User;Hardware;Points;Units;Points to Leader;Points to Next".split(";"),c="rank displayName hardware multipliedPoints units diffToLeader diffToNext".split(" ");categoryLeaderboardTitle=document.createElement("h2");categoryLeaderboardTitle.setAttribute("class","navbar-brand");categoryLeaderboardTitle.innerHTML="Category Leaderboard";
categoryDiv.append(categoryLeaderboardTitle);Object.keys(a).forEach(function(e){var f=getCategoryFrontend(e);categoryTitle=document.createElement("h2");categoryTitle.setAttribute("class","navbar-brand");categoryTitle.innerHTML=f;categoryDiv.append(categoryTitle);tableId="category_"+e.replace(/\s+/g,"_").toLowerCase();categoryTable=document.createElement("table");categoryTable.setAttribute("id",tableId);categoryTable.setAttribute("class","table table-dark table-striped table-hover");categoryTableHead=
document.createElement("thead");categoryTableHeaderRow=document.createElement("tr");b.forEach(function(a,b){categoryTableHeader=document.createElement("th");categoryTableHeader.setAttribute("onclick","sortTable("+b+", '"+tableId+"')");categoryTableHeader.setAttribute("scope","col");categoryTableHeader.innerHTML=a;categoryTableHeaderRow.append(categoryTableHeader)});categoryTableHead.append(categoryTableHeaderRow);categoryTable.append(categoryTableHead);categoryTableBody=document.createElement("tbody");
users=a[e];users.forEach(function(a){categoryTableBodyRow=document.createElement("tr");c.forEach(function(b){categoryCell=document.createElement("td");"multipliedPoints"===b&&(categoryCell.setAttribute("data-bs-toggle","tooltip"),categoryCell.setAttribute("data-placement","top"),categoryCell.setAttribute("title","Unmultiplied: "+a.points.toLocaleString()),new bootstrap.Tooltip(categoryCell));categoryCell.innerHTML=a[b].toLocaleString();categoryTableBodyRow.append(categoryCell)});categoryTableBody.append(categoryTableBodyRow)});
categoryTable.append(categoryTableBody);categoryDiv.append(categoryTable);categoryDiv.append(document.createElement("br"))})})}
function loadTeamStats(){fetch(ROOT_URL+"/stats").then(function(a){return a.json()}).then(function(a){var b="Rank User Category Hardware Points Units".split(" "),c="rankInTeam displayName category hardware multipliedPoints units".split(" ");a=a.teams;a.sort(sortJsonByKey("rank"));a.forEach(function(a,f){var d=f+1,e=a.captainName;teamDiv=document.createElement("div");teamDiv.setAttribute("id","team_"+d+"_div");metadataDiv=document.createElement("div");metadataDiv.setAttribute("id","team_"+d+"_metadata");
...
"category"===b?teamTableUserCell.innerHTML=getCategoryFrontend(a.category):b in a&&(teamTableUserCell.innerHTML=a[b].toLocaleString());teamTableBodyRow.append(teamTableUserCell)});teamTableBody.append(teamTableBodyRow)});retiredUsers=a.retiredUsers;retiredUsers.sort(sortJsonByKey("rankInTeam"));retiredUsers.forEach(function(a,b){teamTableBodyRow=document.createElement("tr");c.forEach(function(b,c){teamTableUserCell=document.createElement("td");"multipliedPoints"===b?(teamTableUserCell.setAttribute("data-bs-toggle",
"tooltip"),teamTableUserCell.setAttribute("data-placement","left"),teamTableUserCell.setAttribute("title","Unmultiplied: "+a.points.toLocaleString()),teamTableUserCell.innerHTML=a[b].toLocaleString(),new bootstrap.Tooltip(teamTableUserCell)):"category"===b?teamTableUserCell.innerHTML="Retired":b in a&&(teamTableUserCell.innerHTML=a[b].toLocaleString());teamTableBodyRow.append(teamTableUserCell)});teamTableBody.append(teamTableBodyRow)});teamTable.append(teamTableBody);subDiv.append(teamTable);teamDiv.append(subDiv);
statsDiv=document.getElementById("stats_div");statsDiv.append(teamDiv);statsDiv.append(document.createElement("br"))})})}document.addEventListener("DOMContentLoaded",function(a){loadOverallStats();loadTeamLeaderboard();loadCategoryLeaderboard();loadTeamStats();startTimer()});function show(a){document.getElementById(a).style.display="block"}function showElement(a){a.style.display="block"}function hide(a){document.getElementById(a).style.display="none"}
function hideElement(a){a.style.display="none"}function getOperatingSystemFrontend(a){return"linux"===a.toLowerCase()?"Linux":"windows"===a.toLowerCase()?"Windows":a}function getOperatingSystemBackend(a){return"linux"===a.toLowerCase()?"linux":"windows"===a.toLowerCase()?"windows":a}function getCategoryFrontend(a){return"amd_gpu"===a.toLowerCase()?"AMD GPU":"nvidia_gpu"===a.toLowerCase()?"nVidia GPU":"wildcard"===a.toLowerCase()?"Wildcard":a}
function getCategoryBackend(a){return"amd gpu"===a.toLowerCase()?"amd_gpu":"nvidia gpu"===a.toLowerCase()?"nvidia_gpu":"wildcard"===a.toLowerCase()?"wildcard":a}window.dataLayer=window.dataLayer||[];function gtag(){dataLayer.push(arguments)}gtag("js",new Date);gtag("config","G-NTH3EFBH2T");function sortJsonByKey(a){return function(b,c){return b[a]>c[a]?1:b[a]<c[a]?-1:0}}function localGet(a){return localStorage.getItem(a)}function localSet(a,b){localStorage.setItem(a,b)}
function localContains(a){return null!==localGet(a)}function sessionGet(a){return sessionStorage.getItem(a)}function sessionSet(a,b){sessionStorage.setItem(a,b)}function sessionContains(a){return null!==sessionGet(a)}
