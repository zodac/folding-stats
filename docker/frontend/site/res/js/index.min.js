...
function sortTable(a,b){var c,g,e,f,d,h,k,l=0;b=document.getElementById(b);g=!0;for(k="asc";g;){g=!1;c=b.rows;for(e=1;e<c.length-1;e++)if(h=!1,f=c[e].getElementsByTagName("TD")[a],d=c[e+1].getElementsByTagName("TD")[a],"asc"==k)if(isNaN(f.innerHTML)||isNaN(d.innerHTML)){if(f.innerHTML.toLowerCase()>d.innerHTML.toLowerCase()){h=!0;break}}else{if(parseInt(f.innerHTML)>parseInt(d.innerHTML)){h=!0;break}}else if("desc"==k)if(isNaN(f.innerHTML)||isNaN(d.innerHTML)){if(f.innerHTML.toLowerCase()<d.innerHTML.toLowerCase()){h=
!0;break}}else if(parseInt(f.innerHTML)<parseInt(d.innerHTML)){h=!0;break}h?(c[e].parentNode.insertBefore(c[e+1],c[e]),g=!0,l++):0==l&&"asc"==k&&(k="desc",g=!0)}}var NUMBER_OF_UPDATES_PROPERTY_NAME="numberOfUpdates",UPDATE_MINUTE=55;function startTimer(){calculateNumberOfUpdates();updateTimer()}
function calculateNumberOfUpdates(){if(isStorageAvailable){var a=new Date,b=Date.UTC(a.getUTCFullYear(),a.getUTCMonth(),1,0,UPDATE_MINUTE,0,0);seconds=Math.floor((a-b)/1E3);minutes=seconds/60;hours=Math.floor(minutes/60);storageContains(NUMBER_OF_UPDATES_PROPERTY_NAME)&&(a=storageGet(NUMBER_OF_UPDATES_PROPERTY_NAME),a<hours&&(a=hours-a,document.getElementById("toast-update-count").innerHTML=a.toLocaleString(),showToast("toast-update",!0)));storageSet(NUMBER_OF_UPDATES_PROPERTY_NAME,hours)}else console.log("No local storage available!")}
