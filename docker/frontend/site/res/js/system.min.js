...
function adminLogin(){var a=document.getElementById("inputUserName").value,b=document.getElementById("inputPassword").value,c=encode(a,b);document.getElementById("inputUserName").value="";document.getElementById("inputPassword").value="";console.log("Username: "+a);console.log("Password: "+b);console.log("Encoded: "+c);fetch(ROOT_URL+"/login/admin",{method:"POST",cache:"no-cache",headers:{"Content-Type":"application/json"},body:JSON.stringify({encodedUserNameAndPassword:c})}).then(function(a){console.log(a.status)})}
...
teamsTableBody.append(teamsTableBodyRow)});teamsTable.append(teamsTableBody);teamsDiv.append(teamsTable)})}document.addEventListener("DOMContentLoaded",function(a){loadHardware();loadUsers();loadTeams();updateTimer()});function encode(a,b){return btoa(a+":"+b)}function sortJsonByKey(a){return function(b,c){return b[a]>c[a]?1:b[a]<c[a]?-1:0}}
