var ROOT_URL="http://127.0.0.1:8080/folding";function toggleMainButtonStyle(a,b){a=document.getElementById(a);b.contains("collapsed")?(a.classList.add("btn-primary"),a.classList.remove("btn-success")):(a.classList.add("btn-success"),a.classList.remove("btn-primary"))}
...
"");defaultHardwareOption.setAttribute("selected","");defaultHardwareOption.innerHTML="Choose Hardware...";d.append(defaultHardwareOption);a.forEach(function(a,b){hardwareOption=document.createElement("option");hardwareOption.setAttribute("value",a.id);hardwareOption.setAttribute("hardware_id",a.id);hardwareOption.setAttribute("hardware_name",a.hardwareName);hardwareOption.setAttribute("display_name",a.displayName);hardwareOption.setAttribute("operating_system",getOperatingSystemFrontend(a.operatingSystem));
hardwareOption.setAttribute("multiplier",a.multiplier);hardwareOption.innerHTML=a.displayName+" ("+getOperatingSystemFrontend(a.operatingSystem)+")";d.append(hardwareOption)})}})}
...
b=JSON.stringify({hardwareName:b,displayName:c,operatingSystem:getOperatingSystemBackend(d),multiplier:e});show("loader");fetch(ROOT_URL+"/hardware/"+a,{method:"PUT",headers:{"Content-Type":"application/json",Authorization:sessionGet("Authorization")},body:b}).then(function(a){hide("loader");if(200!=a.status)failureToast("Hardware update failed with code: "+a.status),a.json().then(function(a){console.error(JSON.stringify(a,null,2))});else{document.getElementById("hardware_update_id").value="";document.getElementById("hardware_update_name").value=
...
function hideElement(a){a.style.display="none"}function encode(a,b){return btoa(a+":"+b)}function getOperatingSystemFrontend(a){return"linux"===a.toLowerCase()?"Linux":"windows"===a.toLowerCase()?"Windows":a}function getOperatingSystemBackend(a){return"linux"===a.toLowerCase()?"linux":"windows"===a.toLowerCase()?"windows":a}function getCategoryFrontend(a){return"amd_gpu"===a.toLowerCase()?"AMD GPU":"nvidia_gpu"===a.toLowerCase()?"nVidia GPU":"wildcard"===a.toLowerCase()?"Wildcard":a}
function getCategoryBackend(a){return"amd gpu"===a.toLowerCase()?"amd_gpu":"nvidia gpu"===a.toLowerCase()?"nvidia_gpu":"wildcard"===a.toLowerCase()?"wildcard":a}function sortJsonByKey(a){return function(b,c){return b[a]>c[a]?1:b[a]<c[a]?-1:0}}function localGet(a){return localStorage.getItem(a)}function localSet(a,b){localStorage.setItem(a,b)}function localContains(a){return null!==localGet(a)}function sessionGet(a){return sessionStorage.getItem(a)}
