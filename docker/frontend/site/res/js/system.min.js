...
teamsTable.append(teamsTableHead);teamsTableBody=document.createElement("tbody");a.forEach(function(a,e){teamsTableBodyRow=document.createElement("tr");c.forEach(function(b,c){teamsTableBodyCell=document.createElement("td");"forumLink"===b?b in a&&(link=document.createElement("a"),link.setAttribute("href",a[b]),link.innerHTML=a[b],teamsTableBodyCell.append(link)):b in a&&(teamsTableBodyCell.innerHTML=a[b].toLocaleString());teamsTableBodyRow.append(teamsTableBodyCell)});teamsTableBody.append(teamsTableBodyRow)});
teamsTable.append(teamsTableBody);teamsDiv.append(teamsTable)})}document.addEventListener("DOMContentLoaded",function(a){loadHardware();loadUsers();loadTeams();updateTimer()});function sortJsonByKey(a){return function(c,d){return c[a]>d[a]?1:c[a]<d[a]?-1:0}}
