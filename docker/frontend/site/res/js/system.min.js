...
a[b],usersTableBodyCell.append(link)):b in a&&(usersTableBodyCell.innerHTML=a[b].toLocaleString());usersTableBodyRow.append(usersTableBodyCell)});usersTableBody.append(usersTableBodyRow)});usersTable.append(usersTableBody);usersDiv.append(usersTable);a.sort(sortJsonByKey("displayName"));userLists=document.querySelectorAll(".user_list");for(var c=0,d;d=userLists[c];c++){for(;d.firstChild;)d.removeChild(d.lastChild);defaultUserOption=document.createElement("option");defaultUserOption.setAttribute("value",
"");defaultUserOption.setAttribute("disabled","");defaultUserOption.setAttribute("selected","");defaultUserOption.innerHTML="Choose User...";d.append(defaultUserOption);a.forEach(function(a,b){userOption=document.createElement("option");userOption.setAttribute("value",a.id);userOption.setAttribute("user_id",a.id);userOption.setAttribute("user_folding_name",a.foldingUserName);userOption.setAttribute("user_display_name",a.displayName);userOption.setAttribute("user_passkey",a.passkey);userOption.setAttribute("user_category",
a.category);userOption.setAttribute("user_profile_link",a.profileLink);userOption.setAttribute("user_live_stats_link",a.liveStatsLink);userOption.setAttribute("user_hardware_id",a.hardwareId);userOption.setAttribute("user_team_id",a.teamId);userOption.setAttribute("user_is_captain",a.userIsCaptain);userOption.innerHTML=a.displayName;d.append(userOption)})}})}
