...
function manualUpdate(){show("loader");fetch(ROOT_URL+"/tc_stats/manual",{headers:{"Content-Type":"application/json",Authorization:sessionGet("Authorization")},body:requestData}).then(function(a){hide("loader");200!=a.status?(failureToast("Manual update failed with code: "+a.status),a.json().then(function(a){console.error(JSON.stringify(a,null,2))})):successToast("Stats manually updated")}).catch(function(a){console.error("Unexpected error updating stats: ",a);return!1})}
function manualReset(){show("loader");fetch(ROOT_URL+"/tc_stats/reset",{headers:{"Content-Type":"application/json",Authorization:sessionGet("Authorization")},body:requestData}).then(function(a){hide("loader");200!=a.status?(failureToast("Manual reset failed with code: "+a.status),a.json().then(function(a){console.error(JSON.stringify(a,null,2))})):successToast("Stats manually reset")}).catch(function(a){console.error("Unexpected error resetting stats: ",a);return!1})}
...
"col");hardwareTableHeader.innerHTML=a;hardwareTableHeaderRow.append(hardwareTableHeader)});hardwareTableHead.append(hardwareTableHeaderRow);hardwareTable.append(hardwareTableHead);hardwareTableBody=document.createElement("tbody");a.forEach(function(a,c){hardwareTableBodyRow=document.createElement("tr");b.forEach(function(e,b){hardwareTableBodyCell=document.createElement("td");hardwareTableBodyCell.innerHTML="multiplier"===e?"x"+a[e].toLocaleString():"operatingSystem"===e?getOperatingSystem(a.operatingSystem):
a[e].toLocaleString();hardwareTableBodyRow.append(hardwareTableBodyCell)});hardwareTableBody.append(hardwareTableBodyRow)});hardwareTable.append(hardwareTableBody);hardwareDiv.append(hardwareTable);a.sort(sortJsonByKey("hardwareName"));hardwareLists=document.querySelectorAll(".hardware_list");for(var c=0,d;d=hardwareLists[c];c++){for(;d.firstChild;)d.removeChild(d.lastChild);defaultHardwareOption=document.createElement("option");defaultHardwareOption.setAttribute("value","");defaultHardwareOption.setAttribute("disabled",
"");defaultHardwareOption.setAttribute("selected","");defaultHardwareOption.innerHTML="Choose Hardware...";d.append(defaultHardwareOption);a.forEach(function(a,b){hardwareOption=document.createElement("option");hardwareOption.setAttribute("value",a.id);hardwareOption.setAttribute("hardware_id",a.id);hardwareOption.setAttribute("hardware_name",a.hardwareName);hardwareOption.setAttribute("display_name",a.displayName);hardwareOption.setAttribute("operating_system",getOperatingSystem(a.operatingSystem));
hardwareOption.setAttribute("multiplier",a.multiplier);hardwareOption.innerHTML=a.displayName+" ("+getOperatingSystem(a.operatingSystem)+")";d.append(hardwareOption)})}})}
...
JSON.stringify({hardwareName:b,displayName:c,operatingSystem:d,multiplier:e});show("loader");fetch(ROOT_URL+"/hardware/"+a,{method:"PUT",headers:{"Content-Type":"application/json",Authorization:sessionGet("Authorization")},body:b}).then(function(a){hide("loader");if(200!=a.status)failureToast("Hardware update failed with code: "+a.status),a.json().then(function(a){console.error(JSON.stringify(a,null,2))});else{document.getElementById("hardware_update_id").value="";document.getElementById("hardware_update_name").value=
"";document.getElementById("hardware_update_display_name").value="";document.getElementById("hardware_update_operating_system").value="";document.getElementById("hardware_update_multiplier").value="";hardwareFields=document.querySelectorAll(".hardware_update");a=0;for(var b;b=hardwareFields[a];a++)hideElement(b);successToast("Hardware '"+c+"' updated");loadHardware()}}).catch(function(a){console.error("Unexpected error updating hardware: ",a);return!1})}
...
b.status),b.json().then(function(a){console.error(JSON.stringify(a,null,2))})):(document.getElementById("team_create_name").value="",document.getElementById("team_create_description").value="",document.getElementById("team_create_forum_link").value="",successToast("Team '"+a+"' created"),loadTeams())}).catch(function(a){console.error("Unexpected error creating team: ",a);return!1})}
...
"";teamFields=document.querySelectorAll(".team_update");a=0;for(var c;c=teamFields[a];a++)hideElement(c);successToast("Team '"+b+"' updated");loadTeams()}}).catch(function(a){console.error("Unexpected error updating team: ",a);return!1})}
...
a.status),a.json().then(function(a){console.error(JSON.stringify(a,null,2))});else{document.getElementById("team_delete_id").value="";document.getElementById("team_delete_name").value="";document.getElementById("team_delete_description").value="";document.getElementById("team_delete_forum_link").value="";teamFields=document.querySelectorAll(".team_delete");a=0;for(var c;c=teamFields[a];a++)hideElement(c);successToast("Team '"+b+"' deleted");loadTeams()}}).catch(function(a){console.error("Unexpected error deleting team: ",
a);return!1})}
...
function hideElement(a){a.style.display="none"}function encode(a,b){return btoa(a+":"+b)}function getOperatingSystem(a){return"linux"===a.toLowerCase()?"Linux":"windows"===a.toLowerCase()?"Windows":a}function getCategory(a){return"amd_gpu"===a.toLowerCase()||"amd gpu"===a.toLowerCase()?"AMD GPU":"nvidia_gpu"===a.toLowerCase()||"nvidia gpu"===a.toLowerCase()?"nVidia GPU":"wildcard"===a.toLowerCase()?"Wildcard":a}function sortJsonByKey(a){return function(b,c){return b[a]>c[a]?1:b[a]<c[a]?-1:0}}
