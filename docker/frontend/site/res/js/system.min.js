...
b=JSON.stringify({hardwareName:b,displayName:c,operatingSystem:d,multiplier:e});show("loader");fetch(ROOT_URL+"/hardware/"+a,{method:"PUT",headers:{"Content-Type":"application/json",Authorization:sessionGet("Authorization")},body:b}).then(function(a){hide("loader");if(200!=a.status)failureToast("Hardware update failed with code: "+a.status),a.json().then(function(a){console.error(JSON.stringify(a,null,2))});else{document.getElementById("hardware_update_id").value="";document.getElementById("hardware_update_name").value=
"";document.getElementById("hardware_update_display_name").value="";document.getElementById("hardware_update_operating_system").value="";document.getElementById("hardware_update_multiplier").value="";hardwareFields=document.querySelectorAll(".hardware_update");a=0;for(var b;b=hardwareFields[a];a++)hideElement(b);successToast("Hardware '"+c+"' updated");loadHardware()}}).catch(function(a){hide("loader");console.error("Unexpected error updating hardware: ",a);return!1})}
...
successToast("Hardware '"+b+"' deleted");loadHardware()}}).catch(function(a){hide("loader");console.error("Unexpected error deleting hardware: ",a);return!1})}
...
function hideElement(a){a.style.display="none"}function encode(a,b){return btoa(a+":"+b)}function getOperatingSystem(a){return"linux"===a.toLowerCase()?"Linux":"windows"===a.toLowerCase()?"Windows":a}function getCategory(a){return"amd_gpu"===a.toLowerCase()?"AMD GPU":"nvidia_gpu"===a.toLowerCase()?"nVidia GPU":"wildcard"===a.toLowerCase()?"Wildcard":a}function sortJsonByKey(a){return function(b,c){return b[a]>c[a]?1:b[a]<c[a]?-1:0}}function localGet(a){return localStorage.getItem(a)}
function localSet(a,b){localStorage.setItem(a,b)}function localContains(a){return null!==localGet(a)}function sessionGet(a){return sessionStorage.getItem(a)}function sessionSet(a,b){sessionStorage.setItem(a,b)}function sessionContains(a){return null!==sessionGet(a)}
