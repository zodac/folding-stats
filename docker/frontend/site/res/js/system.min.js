...
function loadUsers(){fetch(ROOT_URL+"/users").then(function(a){return a.json()}).then(function(a){a.sort(sortJsonByKey("id"));var b="id displayName foldingUserName passkey category profileLink liveStatsLink hardwareId teamId userIsCaptain".split(" ");for(usersDiv=document.getElementById("users_div");usersDiv.firstChild;)usersDiv.removeChild(usersDiv.lastChild);usersTitle=document.createElement("h2");usersTitle.setAttribute("class","navbar-brand");usersTitle.innerHTML="Users";usersDiv.append(usersTitle);
usersTable=document.createElement("table");usersTable.setAttribute("id","users");usersTable.setAttribute("class","table table-dark table-striped table-hover");usersTableHead=document.createElement("thead");usersTableHeaderRow=document.createElement("tr");"ID;User;Folding Name;Passkey;Category;Profile Link;Live Stats Link;Hardware ID;Team ID;Is Captain".split(";").forEach(function(a,b){usersTableHeader=document.createElement("th");usersTableHeader.setAttribute("onclick","sortTable("+b+", 'users')");
usersTableHeader.setAttribute("scope","col");usersTableHeader.innerHTML=a;usersTableHeaderRow.append(usersTableHeader)});usersTableHead.append(usersTableHeaderRow);usersTable.append(usersTableHead);usersTableBody=document.createElement("tbody");dataListOfUsers=document.getElementById("available_users");selectOfUsers=document.getElementById("create_team_users");a.forEach(function(a,c){usersTableBodyRow=document.createElement("tr");b.forEach(function(b,c){usersTableBodyCell=document.createElement("td");
"liveStatsLink"===b||"profileLink"===b?b in a&&(link=document.createElement("a"),link.setAttribute("href",a[b]),link.innerHTML=a[b],usersTableBodyCell.append(link)):b in a&&(usersTableBodyCell.innerHTML=a[b].toLocaleString());usersTableBodyRow.append(usersTableBodyCell)});usersTableBody.append(usersTableBodyRow);c=document.createElement("option");c.setAttribute("value",a.id);c.innerHTML=a.displayName;dataListOfUsers.append(c);c=document.createElement("option");c.setAttribute("value",a.id);c.innerHTML=
a.displayName;selectOfUsers.append(c)});usersTable.append(usersTableBody);usersDiv.append(usersTable);a.sort(sortJsonByKey("displayName"));userLists=document.querySelectorAll(".user_list");for(var c=0,d;d=userLists[c];c++){for(;d.firstChild;)d.removeChild(d.lastChild);defaultUserOption=document.createElement("option");defaultUserOption.setAttribute("value","");defaultUserOption.setAttribute("disabled","");defaultUserOption.setAttribute("selected","");defaultUserOption.innerHTML="Choose User...";d.append(defaultUserOption);
a.forEach(function(a,b){userOption=document.createElement("option");userOption.setAttribute("value",a.id);userOption.setAttribute("user_id",a.id);userOption.setAttribute("user_folding_name",a.foldingUserName);userOption.setAttribute("user_display_name",a.displayName);userOption.setAttribute("user_passkey",a.passkey);userOption.setAttribute("user_category",a.category);userOption.setAttribute("user_profile_link",a.profileLink);userOption.setAttribute("user_live_stats_link",a.liveStatsLink);userOption.setAttribute("user_hardware_id",
a.hardwareId);userOption.setAttribute("user_team_id",a.teamId);userOption.setAttribute("user_is_captain",a.userIsCaptain);userOption.innerHTML=a.displayName;d.append(userOption)})}})}
...
document.getElementById("user_create_is_captain").checked=!1,successToast("User '"+b+"' created"),loadUsers())}).catch(function(a){console.error("Unexpected error creating user: ",a);return!1})}
...
g=document.getElementById("user_update_live_stats_link").value.trim(),h=document.getElementById("user_update_hardware_selector").value.trim(),k=document.getElementById("user_update_team_selector").value.trim(),l=selectedElement.getAttribute("user_is_captain").checked,b=JSON.stringify({foldingUserName:b,displayName:c,passkey:d,category:e,profileLink:f,liveStatsLink:g,hardwareId:h,teamId:k,userIsCaptain:l});show("loader");fetch(ROOT_URL+"/users/"+a,{method:"PUT",headers:{"Content-Type":"application/json",
Authorization:sessionGet("Authorization")},body:b}).then(function(a){hide("loader");200!=a.status?(failureToast("User update failed with code: "+a.status),a.json().then(function(a){console.error(JSON.stringify(a,null,2))})):(document.getElementById("user_update_id").value="",document.getElementById("user_update_folding_name").value="",document.getElementById("user_update_display_name").value="",document.getElementById("user_update_passkey").value="",document.getElementById("user_update_category").value=
"",document.getElementById("user_update_profile_link").value="",document.getElementById("user_update_live_stats_link").value="",document.getElementById("user_update_hardware_selector").value="",document.getElementById("user_update_team_selector").value="",document.getElementById("user_update_is_captain").checked=!1,successToast("User '"+c+"' updated"),loadUsers())}).catch(function(a){console.error("Unexpected error updating user: ",a);return!1})}
...
"";document.getElementById("user_delete_team_selector").value="";document.getElementById("user_delete_is_captain").checked=!1;userFields=document.querySelectorAll(".user_delete");a=0;for(var c;c=userFields[a];a++)hideElement(c);successToast("User '"+b+"' deleted");loadUsers()}}).catch(function(a){console.error("Unexpected error deleting user: ",a);return!1})}
...
document.getElementById("user_delete_is_captain").checked=!0:document.getElementById("user_delete_is_captain").checked=!1;"undefined"!==profileLink?document.getElementById("user_delete_profile_link").value=profileLink:document.getElementById("user_delete_profile_link").value="";"undefined"!==liveStatsLink?document.getElementById("user_delete_live_stats_link").value=liveStatsLink:document.getElementById("user_delete_live_stats_link").value="";userFields=document.querySelectorAll(".user_delete");for(var a=
0,b;b=userFields[a];a++)showElement(b);hide("loader")}
