...
body:a}).then(function(a){hide("loader");201!=a.status?failureToast("Hardware create failed with code: "+a.status):(document.getElementById("hardware_create_name").value="",document.getElementById("hardware_create_display_name").value="",document.getElementById("hardware_create_operating_system").value="",document.getElementById("hardware_create_multiplier").value="",successToast("Hardware '"+b+"' created"),loadHardware())})}
...
b=JSON.stringify({hardwareName:b,displayName:c,operatingSystem:d,multiplier:e});console.log("Updating "+a+", payload: "+b);show("loader");fetch(ROOT_URL+"/hardware/"+a,{method:"PUT",headers:{"Content-Type":"application/json",Authorization:sessionGet("Authorization")},body:b}).then(function(a){hide("loader");200!=a.status?failureToast("Hardware update failed with code: "+a.status):(document.getElementById("hardware_update_id").value="",document.getElementById("hardware_update_name").value="",document.getElementById("hardware_update_display_name").value=
"",document.getElementById("hardware_update_operating_system").value="",document.getElementById("hardware_update_multiplier").value="",successToast("Hardware '"+c+"' updated"),loadHardware())})}
...
a.status);else{document.getElementById("hardware_delete_id").value="";document.getElementById("hardware_delete_name").value="";document.getElementById("hardware_delete_display_name").value="";document.getElementById("hardware_delete_operating_system").value="";document.getElementById("hardware_delete_multiplier").value="";hardwareFields=document.querySelectorAll(".hardware_delete");a=0;for(var c;c=hardwareFields[a];a++)hideElement(c);successToast("Hardware '"+b+"' deleted");loadHardware()}})}
document.addEventListener("DOMContentLoaded",function(a){sessionContains("Authorization")&&(hide("login_form"),show("admin_functions"));loadHardware();loadUsers();loadTeams();updateTimer();hide("loader")});function show(a){document.getElementById(a).style.display="block"}function showElement(a){a.style.display="block"}function hide(a){document.getElementById(a).style.display="none"}function hideElement(a){a.style.display="none"}function encode(a,b){return btoa(a+":"+b)}
