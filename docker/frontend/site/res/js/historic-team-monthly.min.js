var ROOT_URL="http://internal.axihub.ca/folding",selectedUserId=0,selectedUser="",selectedYear=(new Date).getUTCFullYear();
function populateTeamDropdown(){for(var a=document.getElementById("team_dropdown");a.firstChild;)a.removeChild(a.lastChild);fetch(ROOT_URL+"/teams").then(function(a){return a.json()}).then(function(a){teamDropDownDiv=document.getElementById("team_dropdown");a.forEach(function(a,c){teamButton=document.createElement("button");teamButton.setAttribute("class","dropdown-item");teamButton.setAttribute("type","button");teamButton.setAttribute("onclick","getTeamHistoricStats("+a.id+",'"+a.teamName+"')");
teamButton.innerHTML=a.teamName;teamDropDownDiv.append(teamButton)})})}
function getTeamHistoricStats(a,c,b,f,d,e){0!=a&&(selectedTeamId=a);null!=c&&(selectedTeam=c,teamDropdownTitle=document.getElementById("team_dropdown_root"),teamDropdownTitle.innerHTML=selectedTeam);null!=f&&(selectedMonth=f);null!=d&&(selectedMonthName=d);null!=e&&(selectedYear=e,yearDropdownTitle=document.getElementById("year_dropdown_root"),yearDropdownTitle.innerHTML=selectedYear);""!==selectedTeam&&0!==selectedTeamId&&(show("loader"),hide("historic_stats"),fetch(ROOT_URL+"/historic/teams/"+selectedTeamId+
"/"+selectedYear).then(function(a){return a.json()}).then(function(a){for(historicDiv=document.getElementById("historic_stats");historicDiv.firstChild;)historicDiv.removeChild(historicDiv.lastChild);historicTable=document.createElement("table");historicTable.setAttribute("id","historic_table");historicTable.setAttribute("class","table table-dark table-striped table-hover");tableHead=document.createElement("thead");tableHeaderRow=document.createElement("tr");["Month","Points","Units"].forEach(function(a,
b){tableHeader=document.createElement("th");tableHeader.setAttribute("onclick","sortTable("+b+", 'historic_table')");tableHeader.setAttribute("scope","col");tableHeader.innerHTML=a;tableHeaderRow.append(tableHeader)});tableHead.append(tableHeaderRow);historicTable.append(tableHead);tableBody=document.createElement("tbody");a.forEach(function(a,b){tableRow=document.createElement("tr");dateCell=document.createElement("td");dateCell.innerHTML=(new Date(e,a.dateTime.date.month-1,"01")).toLocaleString("default",
{month:"long"});tableRow.append(dateCell);pointsCell=document.createElement("td");pointsCell.setAttribute("data-bs-toggle","tooltip");pointsCell.setAttribute("data-placement","top");pointsCell.setAttribute("title","Unmultiplied: "+a.points.toLocaleString());pointsCell.innerHTML=a.multipliedPoints.toLocaleString();new bootstrap.Tooltip(pointsCell);tableRow.append(pointsCell);unitsCell=document.createElement("td");unitsCell.innerHTML=a.units.toLocaleString();tableRow.append(unitsCell);tableBody.append(tableRow)});
historicTable.append(tableBody);historicDiv.append(historicTable);hide("loader");show("historic_stats")}))}document.addEventListener("DOMContentLoaded",function(a){populateTeamDropdown("team_dropdown");populateYearDropdown("year_dropdown","getTeamHistoricStats");updateTimer()});function show(a){document.getElementById(a).style.display="block"}function showElement(a){a.style.display="block"}function hide(a){document.getElementById(a).style.display="none"}
