...
function populateTeamDropdown(){for(var a=document.getElementById("team_dropdown");a.firstChild;)a.removeChild(a.lastChild);fetch(ROOT_URL+"/teams").then(function(a){return a.json()}).then(function(a){teamDropDownDiv=document.getElementById("team_dropdown");a.forEach(function(a,g){teamButton=document.createElement("button");teamButton.setAttribute("class","dropdown-item");teamButton.setAttribute("type","button");teamButton.setAttribute("onclick","getTeamHistoricStats("+a.id+",'"+a.teamName+"')");
teamButton.innerHTML=a.teamName;teamDropDownDiv.append(teamButton)})})}
function getTeamHistoricStats(a,f){show("loader");hide("historic_stats");var b=(new Date).getFullYear();fetch(ROOT_URL+"/historic/teams/"+a+"/"+b).then(function(a){return a.json()}).then(function(a){dropDownTitle=document.getElementById("team_dropdown_root");dropDownTitle.innerHTML=f;for(historicDiv=document.getElementById("historic_stats");historicDiv.firstChild;)historicDiv.removeChild(historicDiv.lastChild);teamTitle=document.createElement("h1");teamTitle.setAttribute("class","navbar-brand");teamTitle.innerHTML=
f+" ("+b+")";historicDiv.append(teamTitle);historicTable=document.createElement("table");historicTable.setAttribute("id","historic_table");historicTable.setAttribute("class","table table-dark table-striped table-hover");tableHead=document.createElement("thead");tableHeaderRow=document.createElement("tr");["Month","Points","Units"].forEach(function(a,b){tableHeader=document.createElement("th");tableHeader.setAttribute("onclick","sortTable("+b+", 'historic_table')");tableHeader.setAttribute("scope",
"col");tableHeader.innerHTML=a;tableHeaderRow.append(tableHeader)});tableHead.append(tableHeaderRow);historicTable.append(tableHead);tableBody=document.createElement("tbody");a.forEach(function(a,d){tableRow=document.createElement("tr");dateCell=document.createElement("td");dateCell.innerHTML=(new Date(b,a.dateTime.date.month-1,"01")).toLocaleString("default",{month:"long"});tableRow.append(dateCell);pointsCell=document.createElement("td");pointsCell.setAttribute("data-bs-toggle","tooltip");pointsCell.setAttribute("data-placement",
...
document.addEventListener("DOMContentLoaded",function(a){populateTeamDropdown();updateTimer()});function show(a){document.getElementById(a).style.display="block"}function hide(a){document.getElementById(a).style.display="none"}
